...
/* ================= STYLE CARDS PROMPTS ================= */
function getStyleCardPrompts(templateKey) {
...
function getStyleCardNegativePrompt(templateKey) {
    if (templateKey === "anime") {
        return `
...
function getCouplePrompt(templateKey) {
    let scene = "";
...
async function uploadBase64ToFal(imageBase64) {
    ...
    const jpegBuffer = await (0, sharp_1.default)(inputBuffer)
        .rotate()
        .jpeg({ quality: 95 })
        .toBuffer();
...
/* ================= EFFECTS AI CREATE ================= */
async function restyleImage(imageBase64, effect) {
...
/* ================= STYLE CARDS CREATE================= */
async function restyleStyleCardImage(opts) {
