 ...
 */
export const CALCIO_ARCHETYPES = {
  // ===== ARGENTINA =====
...
export function buildCalcioPrompt(opts: {
  archetypeKey: keyof typeof CALCIO_ARCHETYPES;
  presetKey: keyof typeof PRESETS;
  scene: (typeof SCENES_3)[number];
  extra?: string;
}) {
  const archetype = CALCIO_ARCHETYPES[opts.archetypeKey];
